...
export async function POST(request: NextRequest) {
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      //  • $19.00 VIP upsell  → "Purchase"
      //
      // Her iki satış da artık standard "Purchase" event'i olarak fire edilir.
      // (Önceden $19.00 "Subscribe" olarak gönderiliyordu; user kararıyla
      // Purchase'a çevrildi. Custom Conversion ile $19.00 satışları URL filtresi
      ...
      const isVipUpsell = ccyLower === "usd" && amountForCheck === 1900;

      const eventName = "Purchase";
      // eventId prefix'i $19.00 ve 15k TL satışlarını dashboard'da ayırt etmek
